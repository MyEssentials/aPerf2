...
public class ListTileEntitiesExecutor implements CommandExecutor {
    ...
    private static void sendResult(CommandSource src, Filter filter, GroupSupplier group) throws CommandException {
        // Create grouper
        Grouper<TileEntity> grouper = new Grouper<>(filter, group);
        ...
        for (World world : Sponge.getServer().getWorlds()) {
            ...
            if (tileEntities.size() <= 0) continue;

            int total = 0;
            List<Map.Entry<String, Integer>> groups = grouper.group(tileEntities);
